...
from diag.PySipebiDiagAturanPartikelPun import PySipebiDiagAturanPartikelPun
...
class PySipebiDiagAturanPartikelPun_Val(PySipebiMiniValidationBase):
    ...
    diagScriptFileName = 'PySipebiDiagAturanPartikelPun.py'
    sipebiErrorCodes = ['[KE02]', '[KE03]']

    # Shared resources and output-related properties
    hasSharedResources = True
    # file resources that are taken from py\data\ folder
    fileResourceNames = \
        ['PySipebiDiagAturanPartikelPun_common_check.txt',
        'PySipebiDiagAturanPartikelPun_test.txt',
        'diag/data_partikel_pun.txt']
    # the output file name of this validation script
    outputFilename = 'PySipebiDiagAturanPartikelPun_result.txt'
...
if __name__ == "__main__":
    validation_runner = PySipebiCommonValidationRunner(PySipebiDiagAturanPartikelPun_Val)
