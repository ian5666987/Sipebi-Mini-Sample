...
from diag.PySipebiDiagAturanAngkaBilanganBesar import PySipebiDiagAturanAngkaBilanganBesar
...
class PySipebiDiagAturanAngkaBilanganBesar_Val(PySipebiMiniValidationBase):
    ...
    diagScriptFileName = 'PySipebiDiagAturanAngkaBilanganBesar_Val.py'
    sipebiErrorCodes = ['[KG05]']

    # Shared resources and output-related properties
    hasSharedResources = True
    # file resources that are taken from py\data\ folder
    fileResourceNames = \
        ['PySipebiDiagAturanAngkaBilanganBesaR_common_check.txt',
        'PySipebiDiagAturanAngkaBilanganBesar_test.txt']
    # the output file name of this validation script
    outputFilename = 'PySipebiDiagAturanAngkaBilanganBesar_result.txt'
    ...
    def execute_with_shared_resources(self, shared_resources):
        ...
        try:
            ...
            diag_script = PySipebiDiagAturanAngkaBilanganBesar()
            # TODO read test input file here and execute the diagnostics script using the test input file [PySipebiDiagExample_test.txt]
            test_file = shared_resources.get('PySipebiDiagAturanAngkaBilanganBesar_test.txt')
...
if __name__ == "__main__":
    validation_runner = PySipebiCommonValidationRunner(PySipebiDiagAturanAngkaBilanganBesar_Val)
